...
screen choice(items):
    ...
    vbox:
        for i in items:
            textbutton _(i.caption) action i.action
...
screen navigation():
    ...
    if main_menu:
        ...
        vbox:
            ...
            style "navigation_main_menu_vbox"

            textbutton _("Играть") action Start() style "tv_navigation_button" at menu_hover
            textbutton _("Загрузить") action ShowMenu("load") style "tv_navigation_button" at menu_hover
            textbutton _("Настройки") action ShowMenu("preferences") style "tv_navigation_button" at menu_hover
            textbutton _("Об игре") action ShowMenu("about") style "tv_navigation_button" at menu_hover
            textbutton _("Выход") action Quit(confirm=False) style "tv_navigation_button" at menu_hover
...
style navigation_main_menu_vbox is vbox
style tv_navigation_button is navigation_button
style tv_navigation_button_text is navigation_button_text
...
style tv_navigation_button:
    xalign 0.5
    xsize 420
    ysize 70
    background None
    hover_background "#010000"
    left_padding 20

style tv_navigation_button_text:
    size 42
    color "#BFB2B2"
    hover_color "#ffffff"
    selected_color "#BFB2B2"
    xalign 0.0
    text_align 0.0
...
screen main_menu():
    ...
    on "show" action Play("music", "audio/menu_music.mp3")
    ...
    textbutton "RU / EN":
        ...
        ypos 1010
        background None
        text_style "language_toggle_button_text"
        action If(_preferences.language == "english", Language(None), Language("english"))
...
style tv_screen_frame is empty
...
style tv_screen_frame:
    xpos 620
    ypos 180
    xsize 680
    ysize 720
    background Solid("#030806d0")
...
screen about():
    ...
    frame:
        ...
        vbox:
            ...
            text _("ОБ ИГРЕ") size 50 color "#e6ff00"
            text "Home-Through-the-Night"
            text _("Версия 1.0") size 25
            text _("Автор:") size 25
            text "https://github.com/" size 35
            text "Username-Andrey-is-available" size 40 color "#8a2be2"
            text _("Репозиторий набирает 10 stars,") size 25
            text _("выходит патч, если 20 — новая новелла.") size 25
            text _("Ren'Py лайк душевный ловят, орлы") size 25
...
screen save():
    ...
    use tv_file_slots(_("Сохранить"))

screen load():

    tag menu

    use tv_file_slots(_("Загрузить"))

screen tv_file_slots(title):
        ...
        grid 2 2:
            ...
            for i in range(4):
                ...
                button:
                    ...
                    add FileScreenshot(slot) xsize 320 ysize 180

                    text FileTime(slot,
                        format=_("{#file_time}%d %B %Y %H:%M"),
                        empty=_("Пусто")
                    ) size 22
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    ...
                    text _("Музыка")
                    bar value Preference("music volume") xsize 250

                    text _("Звуки")
                    bar value Preference("sound volume") xsize 250

                    text _("Голос")
                    bar value Preference("voice volume") xsize 250
            ...
            hbox:
                ...
                vbox:
                    ...
                    bar value Preference("text speed") xsize 260

                vbox:
                    spacing 5
                    text _("Авточтение")
                    bar value Preference("auto-forward time") xsize 260
...
screen gameplay_hud():
    ...
    frame:
        ...
        xpadding 12
        ypadding 10
        background Solid("#0d0d0db8")

        vbox:
            spacing 8
            text day_with_weekday(day) size 21 color "#f2f2f2"
            text _("Время: [format_game_time(game_minutes)]") size 23 color "#f2f2f2"
            text _("Деньги: [money] ₽") size 21 color "#d9ffd9"

            text _("Инвентарь") size 20 color "#f2f2f2"
            ...
            grid 2 2:
                ...
                for i in range(inventory_capacity):
                    frame:
                        xsize 120
                        ysize 120
                        background Solid("#1f1f1faa")
                        if i < len(inventory):
                            $ item_name = inventory[i]
                            $ icon = inventory_icon_path(item_name)
                            if icon:
                                add icon xalign 0.5 yalign 0.45
                                $ localized_item_name = _(item_name)
                                text "[localized_item_name]" size 14 xalign 0.5 yalign 0.95
                            else:
                                $ localized_item_name = _(item_name)
                                text "[localized_item_name]" size 16 xalign 0.5 yalign 0.5 text_align 0.5
                        else:
                            text "-" size 26 xalign 0.5 yalign 0.5 color "#888"

    timer 12.0 action [Function(update_game_clock), If(deadline_failed, true=Jump("ending_lost_night"), false=NullAction())] repeat True


screen deadline_darkness():
    zorder 110
    if darkness_alpha > 0.0:
        add Solid("#000") alpha darkness_alpha


style language_toggle_button_text is gui_text

style language_toggle_button_text:
    font "fonts/vcr_osd.ttf"
    size 28
    color "#ffffff"
    hover_color "#e6ff00"
...
screen ending_secrets(summary_lines):
    modal True
    zorder 200
    ...
    frame:
        ...
        vbox:
            ...
            for line in summary_lines:
                if line:
                    text line style "ending_secret_text"
                else:
                    null height 28
...
style ending_secret_text is gui_text

style ending_secret_text:
    color "#ff2a2a"
    size 30
    text_align 0.5
    xalign 0.5
