...
init python:
    ...
    def sanitize_return_stack():
        ...
        original_stack = list(getattr(ctx, "return_stack", []) or [])
        kept = []
        removed = 0
        ...
        for entry in original_stack:
            ctx.return_stack = [entry]
            try:
    ...
    def delete_all_saves():
        ...
        for save_data in renpy.list_saved_games(fast=False):
            try:
        ...
        renpy.notify(_("Сохранения удалены"))
        return removed


    def day_with_weekday(day_num):
        tr = renpy.translate_string
        names = {
            1: tr("Понедельник"),
            2: tr("Вторник"),
            3: tr("Среда"),
        ...
        return tr("День {} — {}").format(day_num, names.get(day_num, tr("Неизвестно")))



    def build_endgame_secret_lines(ending_id=""):
        tr = renpy.translate_string
        revealed = tr("раскрыто")
        hidden = tr("не раскрыто")
        success = tr("успешно")
        failed = tr("не успешно")

        toy_status = hidden
        if getattr(store, "day6_used_needles", False):
            toy_status = success
        elif getattr(store, "day6_used_spokes", False):
            toy_status = failed

        guilt_revealed = bool(getattr(store, "day5_understood_death_link", False) or ending_id == "ending_day6_hook")
        clock_revealed = bool(getattr(store, "day4_frozen_minutes", 0) > 0)
        rainbow_revealed = bool(getattr(store, "lina_die", False))

        return [
            tr("Сюжетные секреты:"),
            tr("Вторая личность — {}" ).format(revealed if getattr(store, "day4_split_personality_revealed", False) else hidden),
            tr("Признание вины за убийства — {}" ).format(revealed if guilt_revealed else hidden),
            tr("Зашили игрушку — {}" ).format(toy_status),
            "",
            tr("Сложные секреты:"),
            tr("Секрет с переводом стрелок на часах — {}" ).format(tr("Безликий переводил стрелки часов, чтобы действовать незаметно") if clock_revealed else hidden),
            tr("Секрет радуги — {}" ).format(tr("Безликий сопоставлял ауру людей с цветами радуги") if rainbow_revealed else hidden),
        ]
...
label after_load:
    ...
    if removed_returns > 0:
        $ renpy.notify(_("Старое сохранение обновлено после изменений сценария"))
    return
...
label add_item(item_name):
    if item_name not in inventory:
        ...
        $ translated_item_name = _(item_name)
        $ renpy.say(narr, _("Получен предмет: [translated_item_name]."))
    return
...
label remove_item(item_name):
    if item_name in inventory:
        ...
        $ translated_item_name = _(item_name)
        $ renpy.say(narr, _("Предмет убран: [translated_item_name]."))
    return
